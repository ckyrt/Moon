...
bool BuildingPipeline::ProcessBuilding(const std::string& jsonStr,
    ...
    if (SemanticBuildingParser::ParseFromString(jsonStr, semanticBuilding, semanticError)) {
        parsedFormInput = ExtractBuildingFormInput(semanticBuilding);
        parsedLayoutInput = ExtractBuildingLayoutInput(semanticBuilding);
        formInput = &parsedFormInput;
        layoutInput = &parsedLayoutInput;
    }
    
    // Process the parsed definition
    return ProcessBuildingInternal(definition, formInput, layoutInput, outBuilding, nullptr, false, outError);
}
...
bool BuildingPipeline::ProcessBuildingBestEffort(const std::string& jsonStr,
    ...
    return ProcessBuildingInternal(definition, formInput, layoutInput, outBuilding, &outReport, true, outError);
}

bool BuildingPipeline::ProcessBuilding(const BuildingDefinition& definition,
                                       GeneratedBuilding& outBuilding,
                                       std::string& outError) {
    return ProcessBuildingInternal(definition, nullptr, nullptr, outBuilding, nullptr, false, outError);
}
