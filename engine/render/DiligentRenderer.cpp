...
void DiligentRenderer::BindAlbedoTexture(const std::string& texturePath)
{
    if (texturePath.empty() || !m_pSRB) {
        ...
        if (m_pDefaultWhiteTextureSRV) {
            auto* var = m_pSRB->GetVariableByName(SHADER_TYPE_PIXEL, "g_AlbedoMap");
            if (var) {
                var->Set(m_pDefaultWhiteTextureSRV, SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
            } else {
                MOON_LOG_ERROR("DiligentRenderer", "BindAlbedoTexture: failed to get g_AlbedoMap variable");
            }
        }
        return;
    }
...
{
    if (texturePath.empty() || !m_pSRB) {
        // 绑定默认纹理 (默认: AO=1, Roughness=0.5, Metallic=0)
        MOON_LOG_WARN("DiligentRenderer", "BindARMTexture: texturePath is empty or SRB is null, using default white texture");
        if (m_pDefaultWhiteTextureSRV) {
            auto* var = m_pSRB->GetVariableByName(SHADER_TYPE_PIXEL, "g_ARMMap");
            if (var) {
                var->Set(m_pDefaultWhiteTextureSRV, SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
            } else {
                MOON_LOG_ERROR("DiligentRenderer", "BindARMTexture: failed to get g_ARMMap variable");
            }
        }
        return;
    }
    
    MOON_LOG_INFO("DiligentRenderer", "BindARMTexture: attempting to load texture: %s", texturePath.c_str());
    
    // 加载并缓存纹理 (ARM 纹理应该是线性空间，不是 sRGB)
    auto* texGPU = GetOrCreateTextureResources(texturePath, false);  // false = linear space for ARM
    if (texGPU && texGPU->SRV) {
        auto* var = m_pSRB->GetVariableByName(SHADER_TYPE_PIXEL, "g_ARMMap");
        if (var) {
            var->Set(texGPU->SRV, SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
            MOON_LOG_INFO("DiligentRenderer", "BindARMTexture: successfully bound texture: %s", texturePath.c_str());
        } else {
            MOON_LOG_ERROR("DiligentRenderer", "BindARMTexture: failed to get g_ARMMap variable");
        }
    } else {
        // 加载失败，使用默认纹理
        MOON_LOG_ERROR("DiligentRenderer", "BindARMTexture: failed to load texture: %s, using default white", texturePath.c_str());
        if (m_pDefaultWhiteTextureSRV) {
            auto* var = m_pSRB->GetVariableByName(SHADER_TYPE_PIXEL, "g_ARMMap");
            if (var) {
                var->Set(m_pDefaultWhiteTextureSRV, SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
...
{
    if (texturePath.empty() || !m_pSRB) {
        // 绑定默认纹理（默认法线 = (0.5, 0.5, 1.0) 对应世界空间 (0, 0, 1)）
        MOON_LOG_WARN("DiligentRenderer", "BindNormalTexture: texturePath is empty or SRB is null, using default white texture");
        if (m_pDefaultWhiteTextureSRV) {
            auto* var = m_pSRB->GetVariableByName(SHADER_TYPE_PIXEL, "g_NormalMap");
            if (var) {
                var->Set(m_pDefaultWhiteTextureSRV, SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
            } else {
                MOON_LOG_ERROR("DiligentRenderer", "BindNormalTexture: failed to get g_NormalMap variable");
            }
        }
        return;
    }
    
    MOON_LOG_INFO("DiligentRenderer", "BindNormalTexture: attempting to load texture: %s", texturePath.c_str());
    
    // 加载并缓存纹理 (法线贴图应该是线性空间，不是 sRGB)
    auto* texGPU = GetOrCreateTextureResources(texturePath, false);  // false = linear space for normal maps
    if (texGPU && texGPU->SRV) {
        auto* var = m_pSRB->GetVariableByName(SHADER_TYPE_PIXEL, "g_NormalMap");
        if (var) {
            var->Set(texGPU->SRV, SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
            MOON_LOG_INFO("DiligentRenderer", "BindNormalTexture: successfully bound texture: %s", texturePath.c_str());
        } else {
            MOON_LOG_ERROR("DiligentRenderer", "BindNormalTexture: failed to get g_NormalMap variable");
        }
    } else {
        // 加载失败，使用默认纹理
        MOON_LOG_ERROR("DiligentRenderer", "BindNormalTexture: failed to load texture: %s, using default white", texturePath.c_str());
        if (m_pDefaultWhiteTextureSRV) {
            auto* var = m_pSRB->GetVariableByName(SHADER_TYPE_PIXEL, "g_NormalMap");
            if (var) {
                var->Set(m_pDefaultWhiteTextureSRV, SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
